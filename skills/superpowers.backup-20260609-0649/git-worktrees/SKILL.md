...
### 2. Check .pi/CLAUDE.md

```bash
grep -i "worktree.*director" .pi/CLAUDE.md 2>/dev/null
```
...
If no directory exists and no .pi/CLAUDE.md preference:
...
| Neither exists | Check .pi/CLAUDE.md → Ask user |
...
- **Fix:** Follow priority: existing > .pi/CLAUDE.md > ask
...
- Skip .pi/CLAUDE.md check

**Always:**
- Follow directory priority: existing > .pi/CLAUDE.md > ask
