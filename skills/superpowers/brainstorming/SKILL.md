...
2. **Offer visual companion** (if topic will involve visual questions) — this is its own message, not combined with a clarifying question. > **Pi note:** Visual companion requires browser tooling not yet available in Pi. Skip this step. Files included for reference at `brainstorming/visual-companion.md`.
...
9. **Create worktree** — invoke git-worktrees skill to create isolated workspace
10. **Transition to implementation** — invoke writing-plans skill (inside worktree) to create implementation plan
...
flowchart TD
    ...
    B -->|"yes (skip in Pi)"| C["Ask clarifying questions"]
    B -->|no| C
    C --> D["Propose 2-3 approaches"]
    D --> E["Present design sections"]
    E --> F{User approves design?}
    F -->|no, revise| E
    F -->|yes| G["Write design doc + commit"]
    G --> H["Spec self-review (fix inline)"]
    H --> I{User reviews spec?}
    I -->|changes requested| G
    I -->|approved| J["Invoke git-worktrees skill"]
...
> **Pi note:** The visual companion requires browser automation tooling not currently available in Pi. This section is preserved for reference. In Pi, brainstorming uses terminal-only design discussion. Skip step 2 of the checklist.
...
If they agree to the companion, read the detailed guide at `brainstorming/visual-companion.md`.
