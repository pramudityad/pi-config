...
def get_llm(model: str):
    ...
    if model == "zai":
        ...
        if not api_key:
            raise ValueError(
                "No ZAI key found in ~/.pi/agent/auth.json "
                "or ZAI_API_KEY env var"
            )
        return ChatOpenAI(
            model="glm-5",
        ...
        raise ValueError(
            ...
            "Choose: zai, google, openai, anthropic, browser-use, ollama"
        )


async def run_agent(task, model, headless, max_steps):
...
def main():
    parser = argparse.ArgumentParser(
        description="Run a browser-use agent task"
    )
